...
public class CharacterDriver : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_controller.isGrounded)
        {
            if (direction != 0)
            {
                if (Mathf.Sign(direction) != Mathf.Sign(_velocity.x)) // Sign returns -1, +1, or 0
                {
                    _velocity.x = 0f;
                }
        ...
        Vector3 deltaPosition = new (deltaX, deltaY, 0f); // Don't have to say new Vector3 for some reason
